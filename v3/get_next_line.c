...
/*   Updated: 2020/12/07 17:37:56 by mvan-wij      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	collapse(t_list *lst, char **dst, size_t size)
{
	int		i;
	t_list	*next;
	char	*str;
	...
	*dst = malloc(size + 1);
	...
	while (lst != NULL)
	{
		...
		{
			lst->size -= size + 1;
			ft_memmove(lst->content, lst->content + size + 1, lst->size); // memcpy?
		}
		else
		{
			size -= lst->size;
...
int		fn_for_start(t_list **lst, char **dst)
{
	...
	{
		*dst = malloc(i + 1);
		ft_memmove(dst, (*lst)->content, i); // cpy?
		(*dst)[i] = '\0';
		(*lst)->size -= i + 1;
		ft_memmove((*lst)->content, (*lst)->content + i + 1, (*lst)->size); // cpy?
		return (1);
	}
	return (0);
}
